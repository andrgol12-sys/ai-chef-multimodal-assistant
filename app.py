...
def dish_to_template(dish) -> dict:
    return {
        ...
        "steps": [strip_step_prefix(step) for step in dish.steps],
    }


def output_file_url(path: Path) -> str:
    relative = path.relative_to(OUTPUT_DIR)
    return url_for("serve_output", filepath=str(relative).replace("\\", "/"))
...
def serve_output(filepath: str):
    ...
    if not str(file_path).startswith(str(output_root)) or not file_path.is_file():
        abort(404)
...
def index():
    ...
    use_uncertain = request.form.get("use_uncertain") == "on"
    uploaded_file = request.files.get("photo")

    vision_detected: list[str] = []
    vision_uncertain: list[str] = []
    vision_uncertain_included: list[str] = []
    text_ingredients: list[str] = []
    vision_image: str | None = None
    vision_processing_time: float | None = None
    saved_files: list[Path] = []
    dish_image_path: str | None = None
    dish_image_error: str | None = None
    dishes: list[dict] = []
    final_ingredients: list[str] = []

    try:
        text_ingredients = parse_text_ingredients(ingredients_text)

        if uploaded_file and uploaded_file.filename:
            statuses.append({"label": "Распознаю продукты…", "done": False})
        ...
        if use_uncertain and vision_uncertain:
            vision_uncertain_included = vision_uncertain
        ...
        if not final_ingredients:
            raise ValueError("Укажите ингредиенты текстом или загрузите фото с продуктами.")

        statuses.append({"label": "Генерирую рецепты…", "done": False})
        client = create_llm_client(provider)
        result = client.suggest_dishes(final_ingredients)
        dishes = [dish_to_template(dish) for dish in result.dishes]
        statuses[-1]["done"] = True
        ...
        if result.dishes:
            statuses.append({"label": "Создаю изображение блюда…", "done": False})
            image_path, dish_image_error = generate_dish_image(result.dishes[0])
            if image_path:
                dish_image_path = output_file_url(image_path)
            statuses[-1]["done"] = True
    ...
    return render_template(
        ...
        saved_files=[
            {"name": path.name, "url": output_file_url(path)}
